#!/usr/bin/env bash
# Every deployment path a trained NequIP-backbone model can take, in order.
#
...
step "3. nequip-compile -- LAMMPS pair_nequip target"
...
if [[ "$TS_OK" == yes ]]; then
    run pair_ts "${COMPILE[@]}" "$CKPT" "$OUT"/pair.nequip.pth \
        --device "$DEVICE" --mode torchscript --target pair_nequip
else
...
# ------------------------------------------------------------ accelerations ----
step "6. accelerations -- OpenEquivariance (CUDA only, needs openequivariance)"
if [[ "$DEVICE" != cuda ]]; then
    skip oeq "not a CUDA device"
elif ! has openequivariance; then
    skip oeq "pip install openequivariance"
else
    run oeq_ase "${COMPILE[@]}" "$CKPT" "$OUT"/ase_oeq.nequip.pt2 \
        --device cuda --mode aotinductor --target ase --modifiers enable_OpenEquivariance
    run oeq_pair "${COMPILE[@]}" "$CKPT" "$OUT"/pair_oeq.nequip.pt2 \
        --device cuda --mode aotinductor --target pair_nequip --modifiers enable_OpenEquivariance
fi

step "7. accelerations -- CuEquivariance (CUDA only, needs cuequivariance-torch)"
if [[ "$DEVICE" != cuda ]]; then
    ...
    run cueq_ase "${COMPILE[@]}" "$CKPT" "$OUT"/ase_cueq.nequip.pt2 \
        --device cuda --mode aotinductor --target ase --modifiers enable_CuEquivariance
    run cueq_pair "${COMPILE[@]}" "$CKPT" "$OUT"/pair_cueq.nequip.pt2 \
        --device cuda --mode aotinductor --target pair_nequip --modifiers enable_CuEquivariance
fi
